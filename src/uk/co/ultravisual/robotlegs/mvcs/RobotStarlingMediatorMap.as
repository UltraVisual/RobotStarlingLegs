...
package uk.co.ultravisual.robotlegs.mvcs
{
    ...
    public class RobotStarlingMediatorMap extends ViewMapBase implements IMediatorMap
    {
        ...
        {
            instantiateMediators(event.target);
        }
        ...
        {
            instantiateMediators(e.target);
        }

        public function instantiateMediators(displayObject:*):void
        {
            if (displayObject is DisplayObject || displayObject is starling.display.DisplayObject)
            {
                if (mediatorsMarkedForRemoval[displayObject])
                {
                    delete mediatorsMarkedForRemoval[displayObject];
                    return;
                }
                var viewClassName:String = getQualifiedClassName(displayObject);
                var config:MappingConfig = mappingConfigByViewClassName[viewClassName];
                if (config && config.autoCreate)
                    createMediatorUsing(displayObject, viewClassName, config);
            }
        }
        ...
        {
            removeInstantiatedMediator(e.target);
        }

        private function removeInstantiatedMediator(displayObject:*):void
        {
            if (displayObject is DisplayObject || displayObject is starling.display.DisplayObject)
            {
                var config:MappingConfig = mappingConfigByView[displayObject];
                if (config && config.autoRemove)
                {
                    mediatorsMarkedForRemoval[displayObject] = displayObject;
